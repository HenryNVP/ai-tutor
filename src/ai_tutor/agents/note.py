...
def build_note_agent(
    ...
    instructions = (
        ...
        "   - STEP 2: Generate a descriptive filename based on the topic.\n"
        "     * Format: [topic]_notes.txt (e.g., 'regnet_notes.txt', 'tesla_regnet_notes.txt')\n"
        "     * Extract topic from the notes content or prompt\n"
        "     * Convert topic to lowercase, replace spaces/special chars with underscores\n"
        "   - STEP 3: Call write_text_file with path: 'data/generated/[filename]'\n"
        "   - STEP 4: After write_text_file returns, respond with ONLY: 'Notes saved to data/generated/[actual_file_path]'\n"
        "     * Use the EXACT path returned by write_text_file (it may be auto-renamed if conflict)\n"
        "   - STEP 5: STOP - do not generate any additional text.\n"
        "   - CRITICAL RULES:\n"
